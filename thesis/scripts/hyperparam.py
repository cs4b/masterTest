...
def preprocess_target(target: str, config: dict, n_proc=4):
    ...
    #load model
    vocab = VocGraph.fromFile(os.path.join(ds_path, config["vocab_file"]))
    pretrained = GraphTransformer(voc_trg=vocab, use_gpus=[0])
    pretrained.loadStatesFromFile(os.path.join(ds_path, config["model_pkg"]))
    
    return pretrained, train_ds, test_ds

#per target setup
TARGET_DATA = {}
for tgt, conf in TARGET_CONFIG.items():
    logger.info(f"Preprocessing target {tgt}...")
    agent, train_ds, test_ds = preprocess_target(tgt, conf)
    TARGET_DATA[tgt] = {
        "agent":       agent,
        "train_ds":    train_ds,
        "test_ds":     test_ds,
        "scoring_dir": conf["scoring_dir"]
    }
...
#training
def train_one_run(target_data, time_budget, sample_budget,
                  learning_rate, batch_size, epsilon,
                  run_dir, epochs=10, thresholds=(0.99,)):
    os.makedirs(run_dir, exist_ok=True)
    agent    = target_data["agent"]
    train_ds = target_data["train_ds"]
    test_ds  = target_data["test_ds"]
    sf_path  = target_data["scoring_dir"]
    ...
    explorer.optim = ScheduledOptim(
        Adam(explorer.agent.parameters(), betas=(0.9, 0.98), eps=1e-9),
        learning_rate,
        512
    )
...
def main():
    ...
    if CLEAR_RESULTS:
        for tgt in TARGET_DATA.keys():
            #for cname in ["time","sample"]:
            #    dir_to_clear = os.path.join(RESULTS_ROOT, tgt, cname)
            #    if os.path.exists(dir_to_clear):
            #        shutil.rmtree(dir_to_clear)
            dir_to_clear = os.path.join(RESULTS_ROOT, tgt, "sample")
            if os.path.exists(dir_to_clear):
                shutil.rmtree(dir_to_clear)
    
    targets     = TARGET_DATA.keys()
    constraints = {"sample": (None, 10000)} #orig constraints = {"time":(600,None), "sample":(None,10000)}
    param_grid  = {"learning_rate":[1,0.5], "batch_size":[64,128], "epsilon":[0.05,0.1]}
    n_runs      = 5    
        ...
        for cname, (tb, sb) in constraints.items():
            for params in dict_product(param_grid):
                ...
                for run in range(1, n_runs+1):
                    run_dir = os.path.join(RESULTS_ROOT, tgt, cname, combo, f"run{run}")
                    os.makedirs(run_dir, exist_ok=True)
                    print(f"[{tgt} | {cname} | {combo} | run {run}]")
                    ...
                    try:
                        ...
                        os.makedirs(run_dir, exist_ok=True)
                        log_path = os.path.join(run_dir, "training_molecules.csv")
                        with open(log_path, "a") as log_f:
                            log_f.write(
                                f"# EXCEPTION: time_budget={tb}, "
                                f"sample_budget={sb}, error={e}\n"
                            )
                        continue
