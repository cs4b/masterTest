...
def find_largest_diverse_subset(molecules: List[Dict], 
    ...
    # For small sets, use exact branch-and-bound
    # For larger sets, use greedy (much faster)
    if n <= 30:
...
def evaluate_circles_from_csv(csv_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        circles = find_largest_diverse_subset(molecules)
...
# =====================================================================
# LOGGING SCORER (wraps ModelScorer + enforces budgets + logs molecules)
# =====================================================================

class LoggingScorer(ModelScorer):
    """
    Wraps ModelScorer: enforces budgets AND logs every molecule/score to CSV.
    Budgets count each scored item to guarantee stopping.
    """
...
def preprocess_target(target: str, config: dict, n_proc=4):
    ...
    logger.info(f"Loading SMILES from {frag_path}...")
    try:
        df = pd.read_csv(frag_path, header=None)
        smiles = set(Standardization(n_proc=n_proc).apply(df.iloc[:,0]))
        logger.info(f"  Loaded {len(smiles)} unique SMILES")
    except Exception as e:
        logger.error(f"Failed to load/standardize SMILES: {e}")
        raise
    ...
    logger.info(f"Encoding molecules and splitting into train/test...")
    ...
    try:
        ...
        # Create empty dataset files with rewrite=True
        train_ds = GraphFragDataSet(train_file, rewrite=True)
        test_ds  = GraphFragDataSet(test_file,  rewrite=True)
        
        logger.info(f"  Encoding {len(smiles)} molecules...")
        encoder.apply(list(smiles), encodingCollectors=[test_ds, train_ds])
        
        logger.info(f"  Encoding complete. Train/test files created.")
    except Exception as e:
        logger.error(f"Failed to encode molecules: {e}")
        raise

    # Reload datasets without rewrite
    try:
        train_ds = GraphFragDataSet(train_file, rewrite=False)
        test_ds  = GraphFragDataSet(test_file,  rewrite=False)
        logger.info(f"  Datasets loaded successfully from {train_file} and {test_file}")
    except Exception as e:
...
# ✅ CLEAR RESULTS FIRST - before any preprocessing
if CLEAR_RESULTS:
    logger.info(f"Clearing results directory: {RESULTS_ROOT}")
...
for tgt, conf in TARGET_CONFIG.items():
    try:
        ...
        logger.info(f"✓ Successfully preprocessed {tgt}\n")
    except Exception as e:
        logger.error(f"✗ Failed to preprocess {tgt}: {e}\n")
        PREPROCESSING_ERRORS[tgt] = str(e)
        # Don't raise - continue to try other targets

if PREPROCESSING_ERRORS:
    logger.warning(f"Preprocessing failed for {list(PREPROCESSING_ERRORS.keys())}")
    for tgt, err in PREPROCESSING_ERRORS.items():
        logger.warning(f"  {tgt}: {err}")

if not TARGET_DATA:
    raise RuntimeError("Preprocessing failed for all targets!")

# Verify all TSV files exist before starting trials
...
for tgt, td in TARGET_DATA.items():
    ...
    if not os.path.exists(train_file):
        raise FileNotFoundError(f"Train file missing after preprocessing: {train_file}")
    if not os.path.exists(test_file):
        raise FileNotFoundError(f"Test file missing after preprocessing: {test_file}")
    
    logger.info(f"  ✓ {tgt}: train={train_file}, test={test_file}")

logger.info("All preprocessing complete and verified.\n")
...
def sample_hyperparams(rng: np.random.Generator) -> Dict[str, Any]:
    ...
    - learning_rate: log-uniform [1e-4, 1.0]
    - batch_size: random int [64, 512], snapped to multiple of 16
...
def train_one_run(target: str, td: dict, time_budget, sample_budget,
    ...
    """
    Train one run with experience replay (reseeding).
    After every epoch, extracts high-scoring molecules and adds them to training set.
    """
    ensure_dir(run_dir)
    set_seed(seed)
...
# =====================================================================
# DATASET RELOADING (for fresh trials/repeats without re-encoding)
# =====================================================================

def reset_train_dataset_fresh(train_file: str, test_file: str):
    """
    Reload fresh train/test datasets from existing TSV files.
    Does NOT re-encode; just resets the objects to point to base molecules only.
    """
    if not os.path.exists(train_file):
    ...
    try:
        ...
        logger.error(f"Failed to load datasets from {train_file} and {test_file}: {e}")
        raise
...
def run_trials_for_combo(tgt: str, td: dict, constraint_name: str,
    ...
    """
    Run N hyperparameter trials with distribution sampling.
    Each trial gets a fresh train/test dataset (no reseeded molecules from previous trials).
    """
    # VERIFY files exist BEFORE starting trials
    if not os.path.exists(td["train_file"]):
        raise FileNotFoundError(
            f"Train file does not exist before trials: {td['train_file']}\n"
            f"Preprocessing may have failed. Check logs above."
        )
    if not os.path.exists(td["test_file"]):
        raise FileNotFoundError(
            f"Test file does not exist before trials: {td['test_file']}\n"
            f"Preprocessing may have failed. Check logs above."
        )
    ...
    for idx in range(1, num_trials + 1):
        ...
        try:
            # ✅ Get FRESH dataset objects from existing TSV files (NO re-encoding)
...
def repeat_best_config(best: dict, tgt: str, constraint_name: str,
    ...
    """
    Re-run best hyperparameters with N independent seeds.
    Each repeat gets a fresh train/test dataset (no reseeded molecules from previous repeats).
    """
    if best is None:
        return None
    ...
    for i, seed in enumerate(seeds, 1):
        ...
        try:
            ...
            # ✅ train_one_run() handles reseeding WITHIN this repeat across epochs
            # ✅ Next repeat gets fresh dataset, so no cross-contamination
            ...
            print(f"    ✗ Unexpected error in repeat {i}: {type(e).__name__}: {e}")
            continue

    # Compute statistics for #CIRCLES (primary metric)
    ...
    print(f"\n[{now()}] Best config summary (with reseeding):")
...
def main():
    ...
    for tgt, td in TARGET_DATA.items():
        for cname, lims in CONSTRAINTS.items():
            print(f"\n{'='*60}")
            print(f"=== {tgt.upper()} | constraint={cname} | WITH RESEEDING ===")
            print(f"{'='*60}")

            try:
                # 1) HP trials with reseeding - pick best by #CIRCLES
                ...
                # 2) Re-run best with N seeds with reseeding; measure variance in #CIRCLES
                ...
                logger.error(f"Failed to run benchmark for {tgt}/{cname}: {e}")
                continue
