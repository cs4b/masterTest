...
class LoggingScorer(ModelScorer):
    ...
    def reset_budgets(self):
        """Reset budgets for potential reuse."""
        self._start = None
        self.eval_count = 0
        self.elapsed_time = 0.0
...
def sample_hyperparams(rng: np.random.Generator) -> Dict[str, Any]:
    ...
    - epsilon: 20% chance of exactly 0, otherwise LogUniform(1e-4, 1e-2)
    ...
    # Epsilon: 20% chance of 0, otherwise log-uniform in [1e-4, 1e-2]
    if rng.uniform(0, 1) < 0.2:
        eps = 0.0
    else:
        eps = float(np.exp(rng.uniform(np.log(1e-4), np.log(1e-2))))
