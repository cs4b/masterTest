...
df = pd.read_csv(f'{DATASETS_PATH}/gsksmiles.txt', header=None)  #test fragbase is drd2smiles
print(df.head())
...
train = GraphFragDataSet(f"{DATASETS_ENCODED_PATH}/gsk_train.tsv", rewrite=True)
test = GraphFragDataSet(f"{DATASETS_ENCODED_PATH}/gsk_test.tsv", rewrite=True)

#encode and test print
encoder.apply(list(smiles), encodingCollectors=[test, train])
test_from_file = GraphFragDataSet(f'{DATASETS_ENCODED_PATH}/gsk_test.tsv')
...
sample_budget = 100000 #10k if sample budget
...
model_scorer_drd= ModelScorer(scoringFunctionCall("diverse-hits/data/scoring_functions/gsk3"))
